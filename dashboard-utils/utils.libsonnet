...
(import 'grafana-builder/grafana.libsonnet') {
  ...
  dashboard(title, uid='', service_metric, links_title)::
    super.dashboard(title, uid) + {
      ...
      addCluster(multi=false)::
        if multi then
          self.addMultiTemplate('cluster', service_metric, $._config.per_cluster_label)
        else
          self.addTemplate('cluster', service_metric, $._config.per_cluster_label),

      addNamespace(multi=false)::
        if multi then
          self.addMultiTemplate('namespace', service_metric + '{' + $._config.per_cluster_label + '=~"$cluster"}', 'namespace')
        else
          self.addTemplate('namespace', service_metric + '{' + $._config.per_cluster_label + '=~"$cluster"}', 'namespace'),
      ...
      addClusterSelectorTemplates(multi=true)::
        ...
        if multi then
          d.addMultiTemplate('cluster', service_metric, $._config.per_cluster_label)
          .addMultiTemplate('namespace', service_metric + '{' + $._config.per_cluster_label + '=~"$cluster"}', 'namespace')
        else
          d.addTemplate('cluster', service_metric, $._config.per_cluster_label)
          .addTemplate('namespace', service_metric + '{' + $._config.per_cluster_label + '=~"$cluster"}', 'namespace'),
    },
