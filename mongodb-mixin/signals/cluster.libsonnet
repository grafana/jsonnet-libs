function(this)
  {
    ...
    signals: {
      ...
      balancerRunning: {
        ...
        sources: {
          percona_mongodb: {
            expr: 'mongodb_mongos_sharding_chunks_is_balancer_running{%(queriesSelector)s}',
            legendCustomTemplate: 'Balancer running',
          },
        },
      },
      collectionsInShards: {
      ...
      shardServicesQps: {
        ...
        sources: {
          percona_mongodb: {
            expr: |||
              sum by (rs_nm) (
                irate(mongodb_op_counters_total{%(queriesSelector)s, cl_role="shardsvr", rs_nm=~"$shard", type!="command"}[$__rate_interval])
              )
            |||,
            legendCustomTemplate: '{{rs_nm}}',
          },
        },
      },
      configServicesQps: {
        ...
        sources: {
          percona_mongodb: {
            expr: |||
              sum by (service_name) (
                irate(mongodb_op_counters_total{%(queriesSelector)s, cl_role="configsvr", type!="command"}[$__rate_interval])
              )
            |||,
      ...
      mongosServicesQps: {
        ...
        sources: {
          percona_mongodb: {
            expr: |||
              sum by (service_name) (
                irate(mongodb_op_counters_total{%(queriesSelector)s, cl_role="mongos", type!="command"}[$__rate_interval])
              )
            |||,
            legendCustomTemplate: '{{service_name}}',
          },
      ...
      // Chunks
      shardChunks: {
        name: 'Number of chunks per shard',
      ...
      // Indexes
      indexesPerShard: {
        name: 'Number of indexes per shard',
      ...
      opsByServiceName: {
        ...
        description: 'Operations per second by type for the selected instance. Series that stay at zero over the range are hidden.',
        type: 'raw',
        unit: 'ops',
        sources: {
          percona_mongodb: {
            expr: |||
              sum by (type) (irate(mongodb_op_counters_total{%(queriesSelector)s, type!="command", service_name=~"$service_name"}[$__rate_interval]))
              and
              sum by (type) (increase(mongodb_op_counters_total{%(queriesSelector)s, type!="command", service_name=~"$service_name"}[$__range])) > 0
            |||,
            legendCustomTemplate: '{{type}}',
          },
        },
