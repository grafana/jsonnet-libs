...
function(config, templates, panels) {
  ...
  dockerDashboardSpec:: grafana.dashboard.new(
      'Docker Overview',
  ...
  docker: resource.new(kind, 'docker')
    + resource.withDocs('Dashboard giving an overview of Docker performance')
    + resource.withSpec(self.dockerDashboardSpec),
}