...
local unavailablePartitionsPanel = {
  ...
  targets: [
    prometheus.target(
      'sum by(instance, aerospike_cluster, job) (aerospike_namespace_unavailable_partitions{job=~"$job", aerospike_cluster=~"$aerospike_cluster", instance=~"$instance"})',
      datasource=promDatasource,
...
local deadPartitionsPanel = {
  ...
  targets: [
    prometheus.target(
      'sum by(instance, aerospike_cluster, job) (aerospike_namespace_dead_partitions{job=~"$job", aerospike_cluster=~"$aerospike_cluster", instance=~"$instance"})',
      datasource=promDatasource,
      legendFormat='{{instance}}',
    ),
  ],
...
local cacheReadUtilizationPanel = {
  ...
  targets: [
    prometheus.target(
      'sum by(job, aerospike_cluster, instance) (aerospike_namespace_cache_read_pct{job=~"$job", aerospike_cluster=~"$aerospike_cluster", instance=~"$instance"})',
      datasource=promDatasource,
      legendFormat='{{instance}}',
    ),
  ],
