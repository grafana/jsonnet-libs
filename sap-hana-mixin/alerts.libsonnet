    ...
    groups: [
      {
        ...
        rules: [
          ...
          },
          {
            ...
            expr: |||
              100 * sum without (database_name) (hanadb_disk_total_used_size_mb{%(filteringSelector)s}) / sum without (database_name) (hanadb_disk_total_size_mb{%(filteringSelector)s}) > %(alertsCriticalHighDiskUtilization)s
            ||| % config,
            'for': '5m',
            labels: {
              severity: 'critical',
