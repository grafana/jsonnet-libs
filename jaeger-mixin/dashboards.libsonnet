    ...
    'jaeger-write.json':
      ...
      .addRow(
        ...
        .addPanel(
          ...
          g.qpsPanelErrTotal('jaeger_tracer_reporter_spans_total{result=~"dropped|err"}', 'jaeger_reporter_spans') +
          g.stack
        )
        .addPanel(
          g.panel('% spans dropped') +
          g.queryPanel('sum(rate(jaeger_tracer_reporter_spans_total{result=~"dropped|err"}[1m])) by (namespace) / scalar(sum(rate(jaeger_tracer_reporter_spans_total[1m])))', '{{namespace}}') +
