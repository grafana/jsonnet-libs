    ...
    groups+: [
      {
        ...
        rules: [
          ...
          },
          {
            alert: 'HighCpuUsage',
            expr: |||
              jvm_process_cpu_load > %(alertsCriticalHighCpuUsage5m)s
            ||| % $._config,
            'for': '5m',
            labels: {
              severity: 'critical',
          ...
          },
          {
            ...
            expr: |||
              sum(jvm_memory_usage_used_bytes{area="Heap"}) / sum(jvm_physical_memory_bytes) * 100 > %(alertsCriticalHighMemoryUsage5m)s
            ||| % $._config,
            'for': '5m',
            labels: {
              severity: 'critical',
